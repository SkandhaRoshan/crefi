...
async function getETHPrice() {
  try {
    ...
    const response = await fetch("https://api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies=usd", { signal: controller.signal });
...
async function onCronTrigger(runtime2) {
  runtime2.log("\uD83D\uDE80 CRE Autopilot started");
  const ethPrice = await getETHPrice();
  if (ethPrice) {
    runtime2.log(`\uD83C\uDF10 Real ETH Price (CoinGecko): $${ethPrice}`);
  } else {
    runtime2.log("⚠️ External API blocked in simulator, using fallback value");
  }
  const vaultAssets = 1e6;
  runtime2.log(`Vault Assets: ${vaultAssets}`);
  const aaveApy = 5 + Math.random() * 5;
  const idleApy = 3;
  runtime2.log(`Aave APY: ${aaveApy}`);
  runtime2.log(`Idle APY: ${idleApy}`);
  let chosenStrategy = "Idle";
  if (aaveApy > idleApy) {
    chosenStrategy = "Aave";
  }
  runtime2.log(`Chosen Strategy: ${chosenStrategy}`);
  if (chosenStrategy === "Aave") {
    runtime2.log("\uD83D\uDCC8 Rebalance Triggered");
    runtime2.log("Moving funds → Aave Strategy");
  } else {
    runtime2.log("\uD83D\uDE34 Staying in Idle Strategy");
  }
